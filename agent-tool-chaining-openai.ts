...
const events = tool(
  ...
  },
  {
    ...
    schema: z.object({
      location: z.string().describe("The location to get event information for"),
    }),
  }
);
...
const agent = createAgent({
  ...
  systemPrompt: `You are an intelligent agent that can use tools to answer user queries.
Make sure to always use both tools when necessary to provide accurate answers. Use events tool only with valid location from addressBook tool.`,
});
